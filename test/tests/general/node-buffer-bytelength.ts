...
export default function () {
	...
	assert.equal(Buffer.byteLength(<any>32, 'binary'), 2);
	assert.equal(Buffer.byteLength(<any>NaN, 'utf8'), 3);
	assert.equal(Buffer.byteLength(<any>{}, 'binary'), 15);
	assert.equal((<any>Buffer).byteLength(), 9);
