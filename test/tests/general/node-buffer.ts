...
export default function () {
	...
	buf.fill(10);
	for (var i = 0; i < buf.length; i++) assert.equal(buf[i], 10);

	buf.fill(11, 0, buf.length >> 1);
	for (var i = 0; i < buf.length >> 1; i++) assert.equal(buf[i], 11);
	for (var i = (buf.length >> 1) + 1; i < buf.length; i++) assert.equal(buf[i], 10);

	buf.fill('h');
	for (var i = 0; i < buf.length; i++) assert.equal('h'.charCodeAt(0), buf[i]);

	buf.fill(0);
	for (var i = 0; i < buf.length; i++) assert.equal(0, buf[i]);

	buf.fill(null);
	for (var i = 0; i < buf.length; i++) assert.equal(0, buf[i]);

	buf.fill(1, 16, 32);
	for (var i = 0; i < 16; i++) assert.equal(0, buf[i]);
	for (; i < 32; i++) assert.equal(1, buf[i]);
	for (; i < buf.length; i++) assert.equal(0, buf[i]);
	...
	assert.strictEqual(c.length, copied, 'Expected ' + c.length + ' copied bytes w/o sourceEnd, got: ' + copied);
	for (var i = 0; i < c.length; i++) {
	...
	assert.strictEqual(c.length, copied, 'Expected ' + c.length + ' copied bytes w/o sourceStart, got: ' + copied);
	for (var i = 0; i < c.length; i++) {
		assert.strictEqual(c[i], b[i]);
	}
	...
	c.fill(++cntr);
	var copied = b.copy(c);
	assert.strictEqual(c.length, copied, 'Expected ' + c.length + ' copied bytes w/o targetStart, got: ' + copied);
	for (var i = 0; i < c.length; i++) {
		assert.strictEqual(b[i], c[i]);
	}
	...
	const expected = Math.floor(c.length / 2);
	assert.strictEqual(expected, copied, 'Expected ' + expected + ' copied bytes, got: ' + copied);
	...
	assert.strictEqual(c.length, copied, 'Expected ' + c.length + ' copied bytes (no overrun), got: ' + copied);
	for (var i = 0; i < c.length; i++) {
		assert.strictEqual(b[i], c[i]);
	}
	...
	assert.strictEqual(768, copied, 'Expected 728 copied bytes (same buffer), got: ' + copied);
	...
	let caught_error: NodeJS.ErrnoException = null;
  ...
  var d = c.slice(0, 2);*/

	{
		...
		const size = buffer.write(testValue, 0, <any>'utf8');
	...
	['ucs2', 'ucs-2', 'utf16le', 'utf-16le'].forEach(function (encoding) {
		...
		const size = f.write('あいうえお', <any>encoding);
	...
	const g = new Buffer(<any>arrayIsh);
...

	{
		...
		b = new Buffer(1024);
		let bytesWritten = b.write(expected, 0, <any>'base64');
	...
	[16, 32].forEach(function (bits) {
		...
		assert.equal((<any>buf)['readUInt' + bits + 'BE'](0), 0xffffffff >>> (32 - bits));

		assert.equal((<any>buf)['readUInt' + bits + 'LE'](0), 0xffffffff >>> (32 - bits));

		assert.equal((<any>buf)['readInt' + bits + 'BE'](0), 0xffffffff >> (32 - bits));

		assert.equal((<any>buf)['readInt' + bits + 'LE'](0), 0xffffffff >> (32 - bits));
	});
