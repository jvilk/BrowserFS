...
export default function () {
	...
	BrowserFS.FileSystem.InMemory.Create({}, (e, rootForMfs?) => {
		...
		BrowserFS.FileSystem.MountableFileSystem.Create({}, (e, newmfs?) => {
			...
			assert.equal(realPathSyncResult, '/root/anotherRoot', `Invariant fail: non-linked directly resolved to different path: ${realPathSyncResult}`);

			fs.realpath('/root/anotherRoot', function (err, p) {
				assert.equal(p, '/root/anotherRoot', `Invariant fail: non-linked directly resolved to different path: ${p}`);
			});
					...
					fs.rename('/root/home/secondRoot', '/root/home/anotherFolder', function (err) {
						...
						fs.rmdir('/root/home', function (err) {
							...
							assert.deepEqual(
								...
								['anotherRoot', 'home'],
								'Invariant fail: Directory listings do not contain duplicate items when folder contains mount points w/ same names as existing files/folders.'
							);
