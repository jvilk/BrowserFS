...
export default function () {
	...
	}, 'Should not be able to rename the deletion log.');
	// Move another file over the deletion log.
	assert.throws(function () {
		fs.renameSync('/test/fixtures/files/node/a1.js', logPath);
	}, 'Should not be able to rename a file over the deletion log.');
	// Remove the deletion log.
	assert.throws(function () {
		fs.unlinkSync(logPath);
	}, 'Should not be able to delete the deletion log.');
	// Open the deletion log.
	assert.throws(function () {
		fs.openSync(logPath, 'r');
	}, 'Should not be able to open the deletion log.');
	// Re-write a.js.
