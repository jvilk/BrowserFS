	...
	if (!fs.getRootFS().isReadOnly()) {
		...
		fs.writeFile(filename, s, function (e) {
			...
			fs.readFile(filename, function (e, buffer) {
				...
				assert.equal(expected, buffer.length, 'Buffer length mismatch for ' + filename + ': expected ' + expected + ', got ' + buffer.length);
			});
		});
		...
		fs.writeFile(filename2, buf, function (e) {
			...
			fs.readFile(filename2, function (e, buffer) {
				if (e) throw e;
				ncallbacks++;
				assert.equal(buf.length, buffer.length, 'Buffer length mismatch for ' + filename2 + ': expected ' + buf.length + ', got ' + buffer.length);
			});
		});
