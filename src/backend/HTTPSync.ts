 ...
 */
export default class HTTPSync extends BaseFileSystem implements FileSystem {
  public static readonly Name = "HTTPSync";

  public static readonly Options: FileSystemOptions = {
    baseUrl: {
      type: "string",
      optional: false,
      description: "Used as URL prefix to reach the httpSync backend."
    },
    preferXHR: {
      type: "boolean",
      optional: true,
   ...
   */
  public static Create(opts: HTTPSyncOptions, cb: BFSCallback<HTTPSync>): void {
    if (typeof (opts.baseUrl) === "string") {
      cb(null, new HTTPSync(opts.baseUrl));
    } else {
      cb(new ApiError(ErrorCode.EINVAL, `HTTPSync requires the url to reach the HttpSync backend.`));
    }
  }
  ...
  public readFile(fname: string, encoding: string, flag: FileFlag, cb: BFSCallback<string | Buffer>): void {
    ...
    this.open(fname, flag, 0x1a4, function(err: ApiError, fd?: File) {
      ...
      const fdCast = <NoSyncFile<HTTPSync>> fd;
